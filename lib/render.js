 ...
 * @param {string} [opts.template_insertion=../templates/yadda_insert.dust] - Specifies a path to a template_insertion dust file.  This file controls the layout for inserting steps into an existing step library.  This template should use dust partial `steps` to insert generated steps from template_steps.
 * @param {string} [opts.template_steps=../templates/yadda_steps.dust] - Specifies a path to a template_steps dust file. This file controls the layout and generation of test steps.
  ...
  return through.obj(function gulpYaddaStepsRenderTransform(file, enc, cb) {
    ...
    render(file.contents.toString(), output, function renderCallback(err, output2) {
      ...
      try {
        ...
        self.emit("error", new PluginError(PLUGIN_NAME, err2, {"fileName": file.path}));
      }
      return cb();
    });
  });
